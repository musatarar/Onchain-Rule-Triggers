...
class ConditionTreeField(serializers.JSONField):
    """A rule's condition tree as nested nodes (:mod:`project.app.rules.utils`),
    or null for none. Read from the stored rows; written whole, and judged by
    the model's ``full_clean()`` in services."""

    def get_attribute(self, instance):
        return instance.condition_tree()


class RuleSerializer(serializers.ModelSerializer):
    conditions = ConditionTreeField(required=False, allow_null=True)
